...
в 99; рынков с покупками обеих сторон нет. Обычно он набирает позицию частями.
В среднем получается 7,83 покупки на рынок, медиана — 7. Минимум была 1
покупка, максимум — 34.
...
Сделок после `end_ts_ms` рынка нет.
...
Использовал Codex 5.5 с изменением настроек reasoning в зависимости от сложности задачи.
...
все сделки без пополнений. Выигрыши становятся доступными в момент операции
`REDEEM` исходного кошелька; если погашения в данных нет, выплата не финансирует
последующие сделки. Максимальная просадка считается от этого депозита по рынкам
в порядке их закрытия. Необходимый начальный депозит получился **$7 057,75**,
максимальная просадка — **$6 613,56**, или **93,71% от начального депозита**.
